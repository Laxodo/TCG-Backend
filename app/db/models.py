...
class UserCardDB(SQLModel, table=True):
    ...
    log_activity: list["LogActivityDB"] = Relationship(back_populates="user_card")
...
class LogActivityDB(SQLModel, table=True):
    id: int | None = Field(default=None, primary_key=True)
    id_user: int = Field(index=True, foreign_key="userdb.id")
    id_card: int = Field(index=True, foreign_key="usercarddb.id")
    ...
    user_card: "UserCardDB" = Relationship(back_populates="log_activity")
